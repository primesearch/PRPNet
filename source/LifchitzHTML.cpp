...
void LifchitzHTML::ServerStats(void)
{
   ...
   do
   {
      ...
      if (c == 1)
         ip_Socket->Send("<th scope=\"row\">%d^<var>%d</var>+<var>y</var>^y</th>", b, b);
      else
         ip_Socket->Send("<th scope=\"row\"><var>%d</var>^%d-y^<var>y</var></th>", b, b);
