...
bool     GFNDivisorHTMLGenerator::SendSpecificPage(string thePage)
{
...
void     GFNDivisorHTMLGenerator::PendingTests(void)
{
   ...
   const char* theSelect = "select GFNDRange.n, GFNDRange.kMin, GFNDRange.kMax, " \
      "       GFNDRangeTest.testID, GFNDRangeTest.userID, " \
      "       GFNDRangeTest.machineID, GFNDRangeTest.instanceID " \
      ...
      "order by GFNDRangeTest.testID, GFNDRange.n, GFNDRange.kMin ";
   ...
   do
   {
      ...
      ConvertToScientificNotation(kMin, kMinStr);
      ConvertToScientificNotation(kMax, kMaxStr);
...
void     GFNDivisorHTMLGenerator::DivisorsFound(void)
{
   SQLStatement* sqlStatement;
...
void  GFNDivisorHTMLGenerator::ServerStats(void)
{
   SQLStatement* sqlStatement;
   int64_t     kMin, kMax;
   ...
   const char* theSelect = "select n, kMin, kMax, rangeStatus, divisors " \
      "  from GFNDRange order by n, kMin";
...
void     GFNDivisorHTMLGenerator::SendLinks()
{
...
void  GFNDivisorHTMLGenerator::ConvertToScientificNotation(int64_t valueInt, string& valueStr)
{
   int32_t  eValue = 0, maxE = 3;
   char     tempValue[50];
