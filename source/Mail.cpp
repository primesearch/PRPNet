...
Mail::Mail(const globals_t * const globals, const string &serverName, const uint32_t portID)
: is_FromEmailID(globals->s_EmailID), is_ProjectName(globals->s_ProjectName), ii_ServerType(globals->i_ServerType),
  is_HostName("localhost"), is_FromPassword(globals->s_EmailPassword), ii_ServerName(serverName), ii_ServerPortID(globals->i_PortID)
{
   ip_Log = globals->p_Log;
   ib_SMTPAuthentication = false;
   ib_SMTPVerification = false;

   ip_DBInterface = nullptr;
...
bool     Mail::SendHeader(const string &toEmailID, const string &theSubject)
{
   char    theDate[255];
   ...
   for (uint32_t i=0; i<ii_DestIDCount; i++)
...
bool     Mail::SendHeader(const string &header, const string &text, const uint32_t expectedRC)
{
   int32_t   rc;
...
bool     Mail::PrepareMessage(const string &toEmailID)
{
...
void     Mail::encodeBase64(const string &inString, string &outString)
{
   constexpr const char * const base64 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
   char     array3[3], array4[4];
   int32_t  i, j, index;
   char     tempOut[200];
   const char * const tempIn = inString.c_str();

   memset(tempOut, 0x00, sizeof(tempOut));
   const char *tempPtr = tempIn;

   i = j = index = 0;
   auto bytes = inString.size();
   while (bytes--)
   {
       array3[i++] = *(tempPtr++);
       if (i == 3)
