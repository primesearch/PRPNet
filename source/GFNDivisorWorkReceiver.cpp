...
GFNDivisorWorkReceiver::GFNDivisorWorkReceiver(DBInterface* dbInterface, Socket* theSocket, globals_t* globals,
...
GFNDivisorWorkReceiver::~GFNDivisorWorkReceiver()
{
}

void  GFNDivisorWorkReceiver::ProcessMessage(string theMessage)
{
...
int32_t  GFNDivisorWorkReceiver::ReceiveWorkUnit(string theMessage)
{
   char     tempMessage[200];
   char     name[60];
   ...
   const char* selectSQL = "select kMax from GFNDRangeTest " \
      " where n = ? " \
      "   and kMin = ? " \
      "   and kMax = ? " \
      "   and testId = ? ";
...
int32_t  GFNDivisorWorkReceiver::ReceiveWorkUnit(int32_t n, int64_t kMin, int64_t kMax, int64_t testID)
{
   int32_t  returnCode;
...
int32_t  GFNDivisorWorkReceiver::ProcessWorkUnit(int32_t n, int64_t kMin, int64_t kMax, int64_t testID)
{
   char* theMessage;
   int64_t   endMin, endMax, endTestID;
   bool      abandoned = false, haveStats = false, success;
   int32_t   gotTerminator;
   int32_t   endN, finds = 0;
   double    secondsToTestRange;
   char      theDivisor[ID_LENGTH];
   SQLStatement* sqlStatement;
   const char* updateRange = "update GFNDRange " \
      "   set rangeStatus = 2, " \
      "       divisors = ?, " \
      "       lastUpdateTime = ? " \
      " where n = ? " \
      "   and kMin = ?" \
      "   and kMax = ?";
   const char* updateRangeTest = "update GFNDRangeTest " \
   ...
   while (theMessage)
   {
      ...
      else if (!memcmp(theMessage, "GFNDivisor: ", 12))
      {
         strcpy(theDivisor, theMessage + 12);
...
bool     GFNDivisorWorkReceiver::AbandonTest(int32_t n, int64_t kMin, int64_t kMax, int64_t testID)
{
   bool  success;
   SQLStatement* sqlStatement;
   const char* deleteSQL = "delete from GFNDRangeTest " \
      ...
      "   and kMax = ? " \
      "   and TestID = ?";
   const char* updateSQL = "update GFNRanage " \
      "   set rangeStatus = 0 " \
      " where n = ? " \
      "   and kMin = ? " \
      "   and kMax = ? ";
...
void     GFNDivisorWorkReceiver::LogResults(int32_t n, int64_t kMin, int64_t kMax, int32_t finds)
{
   Log* testLog;
   char name[60];
...
bool     GFNDivisorWorkReceiver::ProcessFind(char* divisor)
{
   Log* findLog;
   bool     success;
   ...
   const char* selectSQL = "select count(*) from GFNDivisor" \
      " where divisor = ? ";
   const char* insertSQL = "insert into GFNDivisor " \
   ...
   if (matchingCount == 0)
   {
      ...
      findLog = new Log(0, "gfn_factors.log", 0, false);
