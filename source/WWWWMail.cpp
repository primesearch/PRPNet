...
void  WWWWMail::MailSpecialResults(void)
{
   ...
   while (selectStatement->FetchRow(false))
   {
      ...
      ip_Log->LogMessage("Sent e-mail for %" PRIu64":(%+d %+d p)", prime, remainder, quotient);
   }
                           ...
                           const int32_t duplicate, const string &machineID, const string &instanceID, const string &searchingProgram)
{
   ...
   if (!remainder && !quotient)
      success = NewMessage(toEmailID, "%s prime %" PRIu64" found by PRPNet!", searchType, prime);
   else
   {
      if (ii_ServerType == ST_WALLSUNSUN)
         success = NewMessage(toEmailID, "%s special instance %" PRIu64" (0 %+d) p was found by PRPNet!",
                              searchType, prime, quotient);
      else
         success = NewMessage(toEmailID, "%s special instance %" PRIu64" (%+d %+d) p was found by PRPNet!",
