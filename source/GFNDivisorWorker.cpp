...
GFNDivisorWorker::GFNDivisorWorker(Log* theLog, TestingProgramFactory* testingProgramFactory,
...
bool  GFNDivisorWorker::ProcessWorkUnit(int32_t& specialsFound, bool inProgressOnly, double& seconds)
{
   bool          wasCompleted;
   workunit_t* wu;
   GFNDivisorWorkUnitTest* wuTest, * wuTestNext;
   ...
   wuTest = (GFNDivisorWorkUnitTest*)wu->m_FirstWorkUnitTest;
   while (wuTest)
   {
      wuTestNext = (GFNDivisorWorkUnitTest*)wuTest->GetNextWorkUnitTest();
...
bool     GFNDivisorWorker::GetWork(void)
{
   char* readBuf;
   int32_t     endLoop;
   ...
   while (readBuf && !endLoop)
   {
      ...
      else if (!memcmp(readBuf, "WorkUnit: ", 10))
      {
         ...
         if (toScan == wasScanned)
         {
            ...
            wu->m_FirstWorkUnitTest = new GFNDivisorWorkUnitTest(ip_Log, ii_ServerType, is_WorkSuffix, wu,
...
void  GFNDivisorWorker::ReturnWork(uint32_t returnOption)
{
   char* readBuf;
...
bool  GFNDivisorWorker::ReturnWorkUnit(workunit_t* wu, bool completed)
{
   char* theMessage;
   bool        accepted;
   GFNDivisorWorkUnitTest* wuTest, * wuNext;
   ...
   else
   {
      ...
      while (wuTest)
      {
         wuNext = (GFNDivisorWorkUnitTest*)wuTest->GetNextWorkUnitTest();
...
void  GFNDivisorWorker::Save(FILE* fPtr)
{
   workunit_t* wu, * wuNext;
...
void  GFNDivisorWorker::Load(string saveFileName)
{
   FILE* fPtr;
   char           line[2000];
...
bool  GFNDivisorWorker::IsWorkUnitCompleted(workunit_t* wu)
{
   WorkUnitTest* wuTest;
...
bool  GFNDivisorWorker::IsWorkUnitInProgress(workunit_t* wu)
{
   WorkUnitTest* wuTest;
